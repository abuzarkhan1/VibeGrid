...
export const AgentLauncher: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto py-2 select-none">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 text-white/60 font-mono text-xs font-semibold uppercase tracking-wider mb-1">
            ...
            <span className="px-2 py-0.5 rounded-md bg-white/10 border border-white/20 text-[11px] text-white/80 font-mono">AGENTS</span>
          </div>
        ...
        <button
          ...
          disabled={isScanning}
          className="px-3 py-1.5 rounded-md bg-white/[0.05] border border-white/10 text-xs flex items-center gap-1.5 text-white/60 hover:text-white hover:bg-white/10 disabled:opacity-50 active:scale-95 cursor-pointer transition-colors"
        >
          <RefreshCw className={`w-3.5 h-3.5 ${isScanning ? 'animate-spin text-white/80' : ''}`} />
      ...
      <div>
        <div className="mb-2">
          <h3 className="text-xs font-semibold text-white/60 uppercase tracking-wider font-mono">
