...
export const WorkspaceSidebar: React.FC<WorkspaceSidebarProps> = ({ isOpen, onToggle }) => {
  ...
  return (
    <>
      <aside style={{ width: sidebarWidth }} className="bg-surface/60 backdrop-blur-md border-r border-white/[0.06] flex flex-col h-full select-none z-20 animate-fade-in shrink-0">
        {/* Sidebar Header */}
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
              ...
              <div
                ...
                className={`group p-2.5 rounded-lg border cursor-pointer transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-forest-bright/70 ${
                  ...
                  dropTargetIdx === idx && dragWsId && dragWsId !== ws.id
                    ? 'ring-2 ring-forest-bright/80 ring-offset-0'
                    : ''
                }`}
              >
                  ...
                  <div className="flex items-center gap-2 truncate">
                    ...
                    {ws.emoji ? (
                      <button
                        ...
                        className="w-4 h-4 text-[13px] leading-none shrink-0 rounded hover:bg-white/10 flex items-center justify-center"
                      >
                        {ws.emoji}
                      </button>
                    ) : (
                      <button
                        ...
                        className="w-4 h-4 shrink-0 rounded hover:bg-white/10 flex items-center justify-center opacity-40 hover:opacity-100"
                      >
          ...
          {archivedWorkspaces.length > 0 && (
            <div className="pt-2 mt-2 border-t border-white/[0.06]">
              <div className="px-2 pb-1 flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-white/30">
                <Archive className="w-3 h-3" /> Archived ({archivedWorkspaces.length})
              </div>
              {archivedWorkspaces.map((ws) => (
                <div
                  key={ws.id}
                  className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-white/[0.04] bg-white/[0.01] text-white/45 mt-1"
                >
                  ...
                  <button
                    ...
                    className="p-1 rounded hover:bg-forest/15 text-forest-light/70 hover:text-forest-light transition-colors"
                  >
